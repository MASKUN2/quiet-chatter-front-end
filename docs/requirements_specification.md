...
- **톡 작성**: 로그인 여부와 관계없이(게스트 자동 승격) 톡을 작성할 수 있는 폼을 제공합니다.
- **톡 리스트**: 해당 도서에 작성된 톡들을 최신순으로 나열합니다.
- **리액션**: 각 톡에 대해 '좋아요', '공감해요' 버튼을 제공하고, 내 반응 여부를 시각적으로 표시합니다.
...
- **사용자 정보**: 현재 로그인된 사용자의 닉네임과 역할을 표시합니다. 모바일에서는 햄버거 메뉴로 축소합니다.
- **VOC**: 사용자 의견을 접수받는 모달 창 호출 버튼을 배치합니다.

## 3. 기술적 요구사항

### 3.1 인증 및 권한
- **투명한 인증**: 별도의 회원가입/로그인 페이지 없이, 활동 시 자동으로 게스트 계정이 생성되고 토큰이 발급되어야 합니다.
- **토큰 관리**: 액세스 토큰 만료 시 리프레시 토큰을 이용해 백그라운드에서 토큰을 갱신(Silent Refresh)해야 합니다.
